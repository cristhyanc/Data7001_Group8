...
export class HomeComponent implements OnInit {
  ...
  public electoraldistrictsDropbox: Electoraldistricts[] = [];
  ...
  constructor(http: HttpClient, @Inject('BASE_URL') baseUrl: string) {
    ...
    http.get<Electoraldistricts[]>(baseUrl + 'data7001/GetElectoraldistricts').subscribe(result => {
      ...
      this.electoraldistrictsDropbox = [... this.electoraldistricts.filter(x => x.year == 2017)];
      this.CreateInitialMapLayers();
    }, error => console.error(error));
  }
  ...
  private DrillMap(newDistrict: string) {
    ...
    if (district.length > 0) {
      this.maps.baseLayerIndex = district[0].position;
        (<HTMLElement>document.getElementById('charBubble')).style.visibility = 'hidden';
  ...
  ngAfterViewInit() {
    ...
    document.getElementById('category').onclick = () => {
      ...
      (<HTMLElement>document.getElementById('charBubble')).style.visibility = 'visible';
